...
async def remark(message: Message, state: FSMContext):
    ...
    # تنظیم marzneshin به عنوان پیش‌فرض و حذف مرحله انتخاب نوع سرور
...
@router.callback_query(
    F.data.in_(["yes", "no"]), StateFilter(ServerCreateForm.SUDO)
)
async def sudo_choice(callback: CallbackQuery, state: FSMContext):
    is_sudo = callback.data == "yes"
    await state.update_data(is_sudo=is_sudo)
...
async def data(message: Message, state: FSMContext):
    ...
    server = await crud.create_server(
        ...
        types=server_type,
        data=server_data,
        is_sudo=state_data.get("is_sudo", True),
    )
