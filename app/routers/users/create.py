...
async def data(callback: CallbackQuery, callback_data: PageCB, state: FSMContext):
    ...
    # برای ادمین غیر سودو، از نام کاربری ادمین در data استفاده می‌کنیم
    if not server.is_sudo:
        # مستقیماً به مرحله بعد می‌رویم و از ادمین فعلی استفاده می‌کنیم
        admin_username = server.data.get("username", "")
        await state.update_data(admin=admin_username)
        ...
        return await callback.message.edit_text(
            ...
            reply_markup=BotKeys.selector(
                ...
                width=1,
                server_back=int(callback_data.panel),
            ),
        )
    
    # برای ادمین سودو، لیست ادمین‌ها را نمایش می‌دهیم
...
async def usercount(message: Message, state: FSMContext):
    ...
    # تنظیم مقدار پیش‌فرض 1 برای suffix و حذف مرحله پرسیدن آن
    await state.update_data(usersuffix="1")
    ...
    if users:
        for user in users:
            ...
            if not user_created:
                ...
                # فقط برای ادمین‌های سودو از set_owner استفاده می‌کنیم
                if server.is_sudo:
