...
class MarzneshinUserResponse(BaseModel):
    ...
    def format_data(self) -> dict:
        ...
        from app.settings.utils.text_translator import translate_expire_strategy, translate_data_limit
        
        expire_strategy_text = f"{self.expire_strategy.value} ({self.get_expire_info(now)})"
        data_limit_text = format_bytes(self.data_limit) if self.data_limit else "Unlimited"
        
        return {
            "username": self.username,
            "expire_strategy": translate_expire_strategy(expire_strategy_text) if expire_strategy_text else expire_strategy_text,
            "activation_deadline": format_date_diff(now, self.activation_deadline),
            "data_limit": translate_data_limit(data_limit_text) if data_limit_text else data_limit_text,
